...
<footer class="tpl-foot">
    @if ($ig || $wa)
        <div class="tpl-social">
            @if ($ig)
                <a href="https://instagram.com/{{ $ig }}" target="_blank" rel="noopener" aria-label="Instagram">
    ...
    @if ($restaurant->address)
        <p class="tpl-address">{{ $restaurant->address }}</p>
    @endif
    <a class="tpl-credit" href="{{ 'https://'.parse_url(config('app.url'), PHP_URL_HOST) }}">{{ config('neva.brand.name') }} ile hazırlandı</a>
</footer>
