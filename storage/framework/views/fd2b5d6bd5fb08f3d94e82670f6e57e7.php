...
<div class="so" x-data="{ qf: 'all' }">
    <header class="so-hero <?php echo e($coverUrl ? '' : 'no-cover'); ?>" data-tpl-cover data-has-cover="<?php echo e($coverUrl ? '1' : ''); ?>"
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.tpl.logo','data' => ['url' => $logoUrl,'name' => $restaurant->name,'class' => 'so-logo']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php endif; ?>
<?php $component->withAttributes(['url' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($logoUrl),'name' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($restaurant->name),'class' => 'so-logo']); ?>
...
<?php endif; ?>
            <h1 class="so-title tpl-h"><?php echo e($restaurant->name); ?></h1>
            <?php if($restaurant->tagline): ?><p class="so-tag"><?php echo e($restaurant->tagline); ?></p><?php endif; ?>
        </div>
    </header>


    <?php if($soFeat || $soDisc): ?>
        <nav class="tpl-quickfilter">
            <button type="button" class="tpl-qf-btn" :class="qf === 'all' && 'is-active'" @click="qf = 'all'">Tüm Menü</button>
            <?php if($soFeat): ?><button type="button" class="tpl-qf-btn" :class="qf === 'feat' && 'is-active'" @click="qf = 'feat'">★ Öne Çıkanlar</button><?php endif; ?>
            <?php if($soDisc): ?><button type="button" class="tpl-qf-btn" :class="qf === 'disc' && 'is-active'" @click="qf = 'disc'">% Fırsatlar</button><?php endif; ?>
        </nav>
    <?php endif; ?>
    ...
    <?php $__empty_1 = true; $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
        ...
        ?>
        <section class="so-cat" x-show="qf === 'all' || (qf === 'feat' && <?php echo e($catFeat ? 'true' : 'false'); ?>) || (qf === 'disc' && <?php echo e($catDisc ? 'true' : 'false'); ?>)">
            <h2 class="so-cat-name tpl-h"><?php echo e($category->name); ?></h2>
            <div class="so-list">
                <?php $__currentLoopData = $category->products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php $img = $p->productImage($product); ?>
                    <article class="so-row" <?php echo flag_attrs($product); ?>

                             x-show="qf === 'all' || (qf === 'feat' && <?php echo e($product->is_featured ? 'true' : 'false'); ?>) || (qf === 'disc' && <?php echo e($product->discount_price ? 'true' : 'false'); ?>)">
                        <div class="so-info">
...
<?php endif; ?>
                            <h3 class="so-name"><?php echo e($product->name); ?></h3>
                            <?php if($showDesc && $product->description): ?><p class="so-desc"><?php echo e($product->description); ?></p><?php endif; ?>
...
<?php $component = Illuminate\View\AnonymousComponent::resolve(['view' => 'components.tpl.price','data' => ['product' => $product,'currency' => $currency,'show' => $showPrices,'class' => 'so-price']] + (isset($attributes) && $attributes instanceof Illuminate\View\ComponentAttributeBag ? $attributes->all() : [])); ?>
...
<?php $attributes = $attributes->except(\Illuminate\View\AnonymousComponent::ignoredParameterNames()); ?>
<?php endif; ?>
<?php $component->withAttributes(['product' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($product),'currency' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($currency),'show' => \Illuminate\View\Compilers\BladeCompiler::sanitizeComponentAttribute($showPrices),'class' => 'so-price']); ?>
<?php echo $__env->renderComponent(); ?>
<?php endif; ?>
...
<?php endif; ?>
                        ...
                        <?php if($img): ?><img class="so-thumb" src="<?php echo e($img); ?>" alt="<?php echo e($product->name); ?>" loading="lazy"><?php endif; ?>
                    </article>
...
<?php /**PATH C:\Users\saidb\Desktop\projeler\nevaqrmenü\resources\views/templates/skeletons/sunset-orange.blade.php ENDPATH**/ ?>